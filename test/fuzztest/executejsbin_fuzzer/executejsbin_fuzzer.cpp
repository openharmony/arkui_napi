...
namespace OHOS {
    void ExecuteJsBinFuzzTest(const uint8_t* data, size_t size)
    {
        if (size <= 0) {
            return;
        }
        ...
        g_nativeEngine.ExecuteJsBin(name);

        const std::string path = "test.abc";
        g_nativeEngine.ExecuteJsBin(path);
    }
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ExecuteJsBinFuzzTest(data, size);
    return 0;
}