...
HWTEST_F(NapiContextTest, CreateLimitRuntimeWithMultiContext001, testing::ext::TestSize.Level1)
{
    BasicDeathTest(
        ...
        []([[maybe_unused]] const std::string& out, const std::string& err) {
...
/**
 * @tc.name: WrapSendableWithMultiContextTest001
 * @tc.desc: Test napi_wrap_sendable_s when designated env is generated by napi_create_ark_context interface.
 * @tc.type: FUNC
 */
HWTEST_F(NapiContextTest, WrapSendableSWithMultiContextTest001, testing::ext::TestSize.Level1)
{
    napi_env env = (napi_env)engine_;
    napi_value exception = nullptr;
    ASSERT_CHECK_CALL(napi_get_and_clear_last_exception(env, &exception));
    ...
    bool isShared = false;
    ASSERT_CHECK_CALL(napi_is_sendable(env, obj, &isShared));
    ASSERT_EQ(isShared, true);

    SetMainEnvContext(false);
    napi_status status = napi_wrap_sendable_s(env, obj, (void*)STRING_TEST, nullptr, nullptr, &wrapTypeTag);
    ASSERT_EQ(status, napi_invalid_arg);
    SetMainEnvContext(true);
}
...
/**
 * @tc.name: WrapSendableWithMultiContextTest002
 * @tc.desc: Test napi_unwrap_sendable_s when designated env is generated by napi_create_ark_context interface.
 * @tc.type: FUNC
 */
HWTEST_F(NapiContextTest, WrapSendableSWithMultiContextTest002, testing::ext::TestSize.Level1)
{
    napi_env env = (napi_env)engine_;
    napi_value obj = nullptr;
    napi_property_descriptor desc[] = {
    ...
    napi_status status = napi_wrap_sendable_s(env, obj, (void*)TEST_WRAP_STRING,
        [](napi_env env, void* data, void* hint) {}, nullptr, &wrapTypeTag);
    ASSERT_EQ(status, napi_ok);

    char* tempTestStr = nullptr;
    SetMainEnvContext(false);
    status = napi_unwrap_sendable_s(env, obj, &wrapTypeTag, (void**)&tempTestStr);
    ASSERT_EQ(status, napi_invalid_arg);
    SetMainEnvContext(true);
}
...
/**
 * @tc.name: WrapEnhanceWithMultiContext001
 * @tc.desc: Test napi_wrap_enhance_s when context is sub context.
 *           interface.
 * @tc.type: FUNC
 */
HWTEST_F(NapiContextTest, WrapEnhanceSWithMultiContext001, testing::ext::TestSize.Level1)
{
    ASSERT_NE(multiContextEngine_, nullptr);
    napi_env env = reinterpret_cast<napi_env>(multiContextEngine_);
    ...
    status = napi_wrap_enhance_s(
        env, obj, (void*)TEST_WRAP_STRING, [](napi_env env, void* data, void* hint) {}, true, nullptr, size,
        &wrapTypeTag, nullptr);
    ASSERT_EQ(status, napi_ok);

    char* tempTestStr = nullptr;
    status = napi_unwrap(env, obj, (void**)&tempTestStr);
    ASSERT_EQ(status, napi_ok);
    ASSERT_STREQ(TEST_WRAP_STRING, tempTestStr);
