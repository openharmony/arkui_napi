...
void ArkIdleMonitor::PostMonitorTask(uint64_t delayMs)
{
    ...
    if (waitForStopTimerHandler_.load() != -1) {
        int ret = ffrt_timer_stop(ffrt_qos_user_initiated, waitForStopTimerHandler_.load());
        if (ret != 0) {
            HILOG_ERROR("ArkIdleMonitor: ffrt_timer_stop error handler: timerHandler='%{public}d', ret='%{public}d'",
                waitForStopTimerHandler_.load(), ret);
        }
    }
    waitForStopTimerHandler_.store(currentTimerHandler_.load());
    currentTimerHandler_.store(ffrt_timer_start(ffrt_qos_user_initiated, delayMs, this, task, false));
#endif
}
...
bool ArkIdleMonitor::CheckWorkerEnvQueueAllInIdle()
{
    ...
    for (size_t i = 0; i < workerEnvQueue_.size(); i++) {
        napi_env env = workerEnvQueue_.front();
        auto arkNativeEngine = reinterpret_cast<ArkNativeEngine*>(env);
        if (arkNativeEngine->GetWorkerThreadState()->GetCheckCount() < checkCounts) {
            return false;
        }
        workerEnvQueue_.pop();
        workerEnvQueue_.push(env);
...
{
#if defined(ENABLE_FFRT)
    // It is possible that the current timer has started to execute, so we need to close it again
    int needStopHandler = currentTimerHandler_.load();
    if (needStopHandler != -1) {
        ffrt_timer_stop(ffrt_qos_user_initiated, needStopHandler);
    }
    if (needStopHandler != currentTimerHandler_.load()) {
        ffrt_timer_stop(ffrt_qos_user_initiated, currentTimerHandler_.load());
    }
    if (waitForStopTimerHandler_.load() != -1) {
        ffrt_timer_stop(ffrt_qos_user_initiated, waitForStopTimerHandler_.load());
    }
#endif
}
