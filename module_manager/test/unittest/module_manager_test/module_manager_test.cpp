...
HWTEST_F(ModuleManagerTest, LoadNativeModuleTest_011, TestSize.Level1)
{
    ...
    EXPECT_EQ(moduleManager->FindNativeModuleByDisk(moduleName, nullptr, nullptr, false, false, errInfo,
        loadErrInfo, nativeModulePath, nullptr), nullptr);
...
HWTEST_F(ModuleManagerTest, LoadNativeModule_ErrInfo_ModuleNameNull, TestSize.Level1)
{
    ...
    std::string loadErrInfo = "";
    NativeModuleManager* moduleManager = NativeModuleManager::GetInstance();
    NativeModule* module = moduleManager->LoadNativeModuleWithErrorInfo(nullptr, nullptr, false, errInfo,
        false, nullptr, loadErrInfo);
    EXPECT_EQ(module, nullptr);
...
{
    GTEST_LOG_(INFO) << "LoadNativeModule_ErrInfo_InvalidPath starts";

    const char* moduleName = "testModule";
    std::string errInfo = "";
    std::string loadErrInfo = "";
    NativeModuleManager* moduleManager = NativeModuleManager::GetInstance();
    NativeModule* module = moduleManager->LoadNativeModuleWithErrorInfo(moduleName, "../test", false, errInfo,
        false, "", loadErrInfo);
    EXPECT_EQ(module, nullptr);
    EXPECT_EQ(loadErrInfo, "invalid path");

    GTEST_LOG_(INFO) << "LoadNativeModule_ErrInfo_InvalidPath end";
}
...
HWTEST_F(ModuleManagerTest, LoadNativeModule_ErrInfo_RelativePathNotContainDotDot, TestSize.Level1)
{
    ...
    std::string loadErrInfo;
    module = moduleManager->LoadNativeModuleWithErrorInfo(moduleName, nullptr, false, errInfo, false,
        "validPath", loadErrInfo);
    EXPECT_EQ(module, nullptr);
...
HWTEST_F(ModuleManagerTest, FindNativeModuleByDisk_ErrInfo_LoadErrInfoNull, TestSize.Level1)
{
    ...
    std::string errInfo = "";
    std::string loadErrInfo;
    // loadErrInfo is a discarded reference; legacy null-pointer crash test no longer applies
    NativeModule* result = moduleManager->FindNativeModuleByDisk("notExistModule", nullptr, "", false, false,
        errInfo, loadErrInfo, nativeModulePath, nullptr);
    EXPECT_EQ(result, nullptr);
